...
- **Note** — `id, content, type(text|voice), category(privat|geschäftlich|offen), pinned, importance, project_id?, due_at?, completed_at?, recurrence?(daily|weekly|monthly), recurrence_until?, audio_path?, transcript?, created_at, updated_at, deleted_at` (mit `due_at`/`completed_at` wird eine Notiz zur Aufgabe)
