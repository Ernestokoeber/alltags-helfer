...
- **Tracking (eigener Tab `/tracking`, P7 Etappe 1):** manuelle Zeit-/Lern-Erfassung (Fach/Modul — Presets AP1/AP2/Security+/Cloud+ + Freitext —, Datum, Dauer in Minuten, Kategorie, optionale Notiz) + **Dashboard**: Kennzahlen Heute/Woche/**Streak**, Balken der letzten 7 Tage, Aufschlüsselung pro Fach (zugleich Filter), Eintragsliste mit Inline-Bearbeiten/Löschen. Reine Aggregationslogik in `tracking.ts` (testbar), DB in `timeEntries.ts` (Schema seit v1 → keine Migration). **Ignoriert bewusst den Sphärenfilter** (Analysesicht). `sourceApp` trennt `manuell` von `pruefungstrainer` (Lern-App via Sync, Etappe 2) — Lern-App-Einträge erhalten ein „Lern-App"-Badge.
...
- **Qualität:** **172 Unit-Tests grün** + **E2E-Smoke (Playwright, Chromium)** über die Kernflüsse (Navigation aller Tabs, Schnellnotiz, Aufgabe, Arbeit-Tab inkl. Pin, Tracking-Eintrag, Sphären-Umschalter) — eigener CI-Workflow `e2e.yml`, **unabhängig vom Deploy**; in der VM via `scripts/vm-e2e.sh` (Ausgabe VM-lokal, da `mkdir` auf dem sshfs-Mount scheitert). Unit: Datenschicht (vitest + fake-indexeddb), Sphären-Logik, Backup/Projekte (inkl. Hierarchie & Aufgaben), **Arbeitsbereich (Kollegen/Support/Pins)**, **Tracking** (`dauerMinuten`/`streak`/`proFach`/`timeEntries`), `openTasks`/`allTasks`/`istAufgabe`, `tagesgruss`/`wmoToWetter`/`faelligeErinnerungen`/`parseMarkdown`/`sucheAlles`/`monatsTage`/`terminInstanzen`/`erledige`/`erinnerungsZeiten`/`tagestipp`, **E2EE-Krypto & Sync-LWW** und UI-Komponenten inkl. Dashboard-Panels, `Wetter`, `NotizModal` & `Markdown` (`@testing-library/svelte` + happy-dom; Select-Bindungen unter **jsdom**, da happy-dom den `:checked`-Selektor für `<option>` nicht unterstützt).
- **Dev/Git:** Multipass-VM `alltagshelfer-dev`, `scripts/vm-dev.sh`; Arbeit direkt auf `main`.

**Noch offen:** **P7 Etappe 2 — Lern-App-Brücke:** PrüfungsTrainer verschlüsselt Lernzeit (gleiches E2EE-Schema, kopierte `crypto.ts`) und pusht sie über `/sync`; am Worker nur die CORS-Origin freischalten. *(P7 Etappe 1 — Tracking-Tab + manuelle Erfassung — umgesetzt; iPhone-Push on-device bestätigt; „Heute"-Tipp + E2E-Smoke umgesetzt.)*
...
- **TimeEntry** (Tracking + Integrations-API) — `id, source_app, activity, category, started_at, ended_at, metadata(json)` (`source_app`: `manuell`/`pruefungstrainer`; `activity`=Fach/Modul, `metadata.note`=Notiz)
